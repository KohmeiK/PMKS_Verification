...
C = circleLineIntersection(B(1), B(2), Mechanism.LinkLength.BC, Mechanism.Joint.C(iteration - 1, 1), Mechanism.Joint.C(iteration - 1, 2), guideAngleDegrees());
...
% Guide angle for this case, in degrees.
%
% Horizontal and offset, as in Offset_Slider_Crank. What is new is that the crank
% cannot complete a revolution: crank 4 plus offset 3 exceeds coupler 6, so the
% slider reaches a limit and the solve reverses. Every other slider case in v1 runs
% a full turn in one direction, so nothing exercised branch selection through a
% reversal on the circle-line path.
function degrees = guideAngleDegrees()
degrees = 0;
end
