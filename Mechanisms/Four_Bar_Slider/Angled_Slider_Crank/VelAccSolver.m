...
eqn1=velSolver(omegaAB,B-A)+velSolver(omegaBCD,C-B)-[V_c*cos(guideAngleRadians()) V_c*sin(guideAngleRadians()) 0]==0;
...
Mechanism.LinVel.Joint.C(iter,:) = [solution.V_c*cos(guideAngleRadians()) solution.V_c*sin(guideAngleRadians()) 0];
end
...
eqn1=accSolver(AngVel.AB,alphaAB, B-A)+accSolver(AngVel.BCD,alphaBCD,C-B)-[A_c*cos(guideAngleRadians()) A_c*sin(guideAngleRadians()) 0]==0;

solution=solve(eqn1,[aBCD A_c]);

% Store all the determined angular accelerations
AngAcc.AB=[0 0 0];
AngAcc.BCD=[0 0 double(solution.aBCD)]; %angular acceleration of BC

linkNames = fieldnames(Mechanism.LinkCoM);
for i = 1:length(linkNames)
    Mechanism.AngAcc.(linkNames{i})(iter,:) = AngAcc.(linkNames{i});
end
Mechanism.LinAcc.Joint.C(iter,:) = [solution.A_c*cos(guideAngleRadians()) solution.A_c*sin(guideAngleRadians()) 0];
end
...
% Guide angle for this case, in radians. Must match PosSolver's
% guideAngleDegrees: the slider's velocity and acceleration are constrained to
% lie along the same track its position is solved on.
function radians = guideAngleRadians()
radians = deg2rad(30);
end
